...
png("figures/ch2/output_fig/fig2_8.png", height = 5, width = 6, units = "in",
    res = 600)
emp_surge |>
  ggplot(aes(x = emp_rat)) +
  geom_density(bw = 0.09) +
  scale_x_continuous(limits = c(0, 2), breaks = seq(0, 2, 0.5)) +
  scale_y_continuous(limits = c(0, 1)) +
  geom_vline(aes(xintercept = median(emp_rat)), col = "gray45",
             lty = "dashed") +
  annotate("text", x = 0.75, y = 0, label = "median = 1.00", color = "gray45",
           size = 3.5) +
  geom_vline(aes(xintercept = mean(emp_rat)), col = "gray45") +
  annotate("text", x = 1.65, y = 0, label = "mean = 1.43", color = "gray45",
           size = 3.5) +
  xlab("Ratio") +
  ylab("Density") +
  theme_bw() +
