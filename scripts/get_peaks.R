...
for (x in seq_along(states)) {
  ...
  saveRDS(peaks_df, paste0("data-raw/peaks/peaks_sep/peaks_", states[x], ".RDS"))
}
...
for (x in seq_along(states)) {
  ...
  peaks_mat <- do.call(rbind, peaks_list2)
  peaks_df <- as.data.frame(peaks_mat)
  saveRDS(peaks_df, paste0("data-raw/peaks/peaks_sep/peaks_", states[x], "_miss.RDS"))
}

# combine peaks dfs for each state
states <- c("CA", "CO", "ID", "MT", "NM", "NV", "OR", "WY", "WA", "AZ", "UT")
for (i in seq_along(states)) {
  peaks_og <- readRDS(paste0("data-raw/peaks/peaks_sep/peaks_", states[i], ".RDS"))
  peaks_miss <- readRDS(paste0("data-raw/peaks/peaks_sep/peaks_", states[i],
                               "_miss.RDS")
  )
  peaks_tot <- rbind(peaks_og, peaks_miss)
  saveRDS(peaks_tot, paste0("data-raw/peaks/peaks_fin/peaks_fin_", states[i], ".RDS"))
}
