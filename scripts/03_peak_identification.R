...
for (x in seq_along(states)) {
  ...
  saveRDS(peaks_df, paste0("data-raw/peaks_og/peaks_", states[x], ".RDS"))
}

# make overall df of peaks for all states
peaks_tot <- vector("list", length = length(states))
for (i in seq_along(states)) {
  temp <- readRDS(paste0("data-raw/peaks_og/peaks_", states[i],
                         ".RDS"))
...
saveRDS(peaks_totdf, "data-raw/peaks_og/peaks_tot.RDS")
